...
import NFT from '../utils/MyToken.json'
  ...
  const mintCharacter = async () => {
    try {
      ...
      if (ethereum) {
        ...
        let nftTx = await nftContract.mintNFT(sampleAddress, sampleUri)
  ...
  const getMintedNFT = async (tokenId) => {
    try {
      ...
      if (ethereum) {
        ...
        tokenUri = 'https://ipfs.io/ipfs' + tokenUri.substr(6)
        let data = await axios.get(tokenUri)
        let meta = data.data
        let imageUrl = 'https://ipfs.io/ipfs' + meta.image.substr(6)
        setMiningStatus(1)
  ...
  return (
    <div className="flex min-h-screen flex-col items-center bg-[#f3f6f4] pt-32 text-[#6a50aa]">
      ...
      <div className="mb-20 mt-4 text-xl font-semibold">
        <a
          href={`https://testnets.opensea.io/assets/mumbai/${nftContractAddress}`}
          target="_blank"
        >
